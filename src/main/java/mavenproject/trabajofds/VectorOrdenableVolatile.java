...
public class VectorOrdenableVolatile {
    
    //Atributos:
        private volatile int numeroDeElementos;
        private volatile int[] vector;
    //End Atributos.
    
    //Constructor:
        public VectorOrdenableVolatile(){
            numeroDeElementos = 0;
            vector = null;
        }
    //End Constructor.
    ...
    //Métodos públicos:
        ...
        public void rellenaVectorEnOrdenInverso(int n){
            ...
            //Rellenamos el vector en orden.
            for(int i = n; i >= 1; i--) vector[n-i] = i;
        }
        public void rellenaVectorAleatoriamente(int n, int max){
            ...
            for(int i = 0; i < n; i++) vector[i] = rnd.nextInt(max + 1);
        }
        public void imprimeVector(){
            System.out.println(this.toString());
        }
        public long ordenaPorBurbuja(){
            //Pedimos el horario de comienzo en milisegundos y lo guardamos en una variable.
            long start = System.nanoTime();
            
            //Ordenamos el array
            burbuja(this.vector);
            
            //Pedimos el horario de finalizacion en milisegundos y lo guardamos en una variable.
            long finish = System.nanoTime();
            
            //Retornamos la diferencia entre los tiempos para saber cuanto tardo el algoritmo.
            return finish - start;
        }
    ...
    //Métodos privados:
        private String vectorAString(){
            ...
            sb.append("Vector Volátil | NumeroDeElementos: ");
            sb.append(this.numeroDeElementos + "\n Elementos: [");
            for(int i = 0; i < this.numeroDeElementos; i++){
                sb.append(vector[i]);
                if(i != this.numeroDeElementos-1)sb.append(",");
            }
            sb.append("]");
            return sb.toString();
        }
    ...
    //Getters:
        public int getNumeroDeElementos(){return this.numeroDeElementos;}
        public int[] getVector(){return this.vector;}
    //End Getters.
        
    //Setters:
        public void setNumeroDeElementos(int numeroDeElementos){this.numeroDeElementos = numeroDeElementos;}
