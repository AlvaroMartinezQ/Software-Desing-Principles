...
public class Login {
    ...
    public void añadirUsuario(Usuario u){
        if(u instanceof Alumno alumno){
            if(alumnos.añadirAlumno(alumno) == 0)
                nAlumnos++;
        }else if (u instanceof Profesor profesor){
            if(profesores.añadirProfesor(profesor) == 0)
                nProfesores++;
        }else if (u instanceof Administrador administrador){
            if(admins.añadirAdministrador(administrador) == 0)
                nAdmins++;
        }else System.out.println("[Error] - Problema al añadir al usuario");
    }
